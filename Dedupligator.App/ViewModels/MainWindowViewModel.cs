...
namespace Dedupligator.App.ViewModels
{
  public partial class MainWindowViewModel : ViewModelBase, IDisposable
  {
    ...
    public string? TrashFolderPath => SelectedFolderPath is null ? null : FileTrashHelper.GetTrashRootPath(SelectedFolderPath);
    ...
    private async Task ScanFolder()
    {
      ...
      finally
      {
        ...
        _findDuplicatesCancellationTokenSource = null;
    ...
    private bool CanExecuteRemoveFiles => SelectedFolderPath is not null && FilePreviews.Any(x => x.MarkedForDeletion);

    private bool CanExecuteOpenTrashFolder => SelectedFolderPath is not null;

    [RelayCommand(CanExecute = nameof(CanExecuteRemoveFiles))]
    private async Task RemoveFiles()
    {
      if (SelectedFolderPath is null || SelectedFileGroup is null)
        return;
      ...
      var trashFolderPath = FileTrashHelper.GetTrashRootPath(SelectedFolderPath);
      var confirmed = await _confirmationDialogService.ConfirmMoveToTrashAsync(itemsToRemove.Count, trashFolderPath);
      if (!confirmed)
        return;

      var movedFilePaths = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

      foreach (var item in itemsToRemove)
      {
        try
        {
          var movedPath = FileTrashHelper.MoveToTrashFolder(item.FilePath, SelectedFolderPath);
          movedFilePaths.Add(item.FilePath);

          _logger.LogInformation(
            "Файл перемещен в папку удаленных: {SourcePath} -> {DestinationPath}",
            item.FilePath,
            movedPath);
        }
        catch (Exception ex)
        {
          _logger.LogError(ex, "Не удалось переместить файл в папку удаленных: {FilePath}", item.FilePath);
          continue;
        }
      ...
      if (movedFilePaths.Count == 0)
        return;

      UpdateGroupsAfterMoveToTrash(movedFilePaths);
    }

    [RelayCommand(CanExecute = nameof(CanExecuteOpenTrashFolder))]
    private async Task OpenTrashFolder()
    {
      if (TrashFolderPath is null)
        return;

      var trashFolderPath = TrashFolderPath;
      Directory.CreateDirectory(trashFolderPath);

      await FileExplorerHelper.OpenFolderAsync(
        trashFolderPath,
        async error =>
        {
          _logger.LogWarning("Ошибка открытия папки удаленных: {ErrorMessage}", error);
    ...
    async partial void OnSelectedFileGroupChanged(DuplicateGroup? oldValue, DuplicateGroup? newValue)
    {
      ...
      var previews = newValue.Files.Select(file => new ImagePreviewViewModel(
        ...
        _loggerFactory.CreateLogger<ImagePreviewViewModel>())).ToList();
    ...
    partial void OnSelectedFolderChanged(IStorageFolder? value)
    {
      OnPropertyChanged(nameof(TrashFolderPath));
      RemoveFilesCommand.NotifyCanExecuteChanged();
      OpenTrashFolderCommand.NotifyCanExecuteChanged();
    }
    ...
    private void UpdateGroupsAfterMoveToTrash(HashSet<string> movedFilePaths)
    {
      if (SelectedFileGroup is null)
        return;

      var updatedFiles = SelectedFileGroup.Files
        .Where(file => !movedFilePaths.Contains(file.FullName))
        .ToList();

      if (updatedFiles.Count < 2)
      {
        var removedGroupIndex = DuplicateGroups.IndexOf(SelectedFileGroup);
        DuplicateGroups.Remove(SelectedFileGroup);
        SelectedFileGroup = removedGroupIndex >= 0 && removedGroupIndex < DuplicateGroups.Count
          ? DuplicateGroups[removedGroupIndex]
          : DuplicateGroups.LastOrDefault();
        return;
      }

      var updatedGroup = new DuplicateGroup(
        GroupName: updatedFiles[0].Name,
        FileCount: updatedFiles.Count,
        TotalSize: updatedFiles.Sum(x => x.Length).ToFileSizeString(),
        Files: updatedFiles);

      var groupIndex = DuplicateGroups.IndexOf(SelectedFileGroup);
      if (groupIndex >= 0)
      {
        DuplicateGroups[groupIndex] = updatedGroup;
      }

      SelectedFileGroup = updatedGroup;
    }
