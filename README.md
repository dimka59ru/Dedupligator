...
Приложение для поиска и безопасного удаления дубликатов изображений с использованием различных методов сравнения, включая нейронные сети.
...
- **Безопасное удаление**: отмеченные файлы переносятся в `.dedupligator-trash` внутри выбранной папки
- **Подтверждение перед удалением**: приложение показывает, куда именно будут перенесены файлы
...
6. Просмотрите найденные дубликаты и отметьте файлы для переноса
7. Подтвердите перенос в `.dedupligator-trash`
8. При необходимости откройте папку удалённых прямо из интерфейса

## 🗑️ Безопасное удаление

Приложение не удаляет отмеченные файлы безвозвратно. Вместо этого они перемещаются в служебную папку:

`<выбранная_папка>\.dedupligator-trash\`

Особенности поведения:

- сохраняется относительная структура папок
- перед переносом показывается подтверждение
- папка `.dedupligator-trash` автоматически исключается из повторного сканирования
- папку удалённых можно открыть прямо из интерфейса
